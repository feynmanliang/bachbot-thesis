...
\mynote{Similarity to language modeling, char-rnn vs word-rnn similar so note-rnn vs chord-rnn should be as well.
  Benefit is that chord vocabulary MUCH larger than natural language}

\mynote{Try mimicking \citet{franklin2006recurrent}}.

\mynote{Motivation: Long-term dependencies are at the heart of what defines a style of music, with
events spanning several notes or bars contributing to the formation of metrical and phrasal
structure \citet{cooper1963rhythmic}.}
...
\mynote{Describe piano roll vs sheet music}
...
\mynote{Motivate quantization with Western music}.
...
\mynote{Describe MIDIs 127 iquantized pitches, isomorphic to musicXML}

\mynote{Describe Bach and chorales, 4 parts, soprano lead with ATB harmony,
...
\begin{figure}[htbp]
    ...
    \caption{Sheet music representation of some bars from a musical score (BWV133.6) with articulation instructions removed.}
    \label{fig:eg-score}
\end{figure}
...
method \citep{denton1997history}. \mynote{Talk about well-tempered tuning and bach}
...
\cref{tab:mode-intervals} illustrates the pitch intervals between adjacent
...
\begin{table}[htpb]
    ...
    \begin{tabular}{lc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Pitch intervals for the two most important modes\citep{freedman2015correlational}. The pitches in a scale can be found by starting at the tonic and successively offsetting by the given pitch intervals.}
...
\mynote{Talk about how view of unrolled RNN as DNN inspires cross-polination e.g.
...
\end{align}
\cref{eq:hidden-state-stationarity} combined with \cref{eq:output-stationarity} imply that $P(\y_t | \h_t)$
...
\nomenclature[z-BPTT]{BPTT}{Backpropogation Through Time\citep{goller1996learning}}
...
Unfortunately, naive implementations of \cref{eq:rnn-ht} and
\cref{eq:rnn-yt} oftentimes suffers from two well known problems: the
...
Many applications of LSTMs \mynote{cite examples} share some common practical
